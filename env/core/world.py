...
class World(object):
  def __init__(self, size, custom, start_position, start_direction_index, food_position):
    ...
    self.world[size - 1][0] = self.WALL
    self.world[size - 1][size - 1] = self.WALL
  ...
  def init_snake(self):
    if not self.custom:
      ...
      start_position = self.world[random.randrange(SNAKE_SIZE, self.size - SNAKE_SIZE)][random.randrange(SNAKE_SIZE, self.size - SNAKE_SIZE)]
      # choose a random direction index
      start_direction_index = random.choice(self.DIRECTIONS)
  ...
  def init_food(self):
    ...
    if not self.custom:
      ...
      # Code needed for checking your project. Just leave it as is
      try:
  ...
  def move_snake(self, action):
    ...
    if self.snake.alive:
      ...
      if new_snake_head[0] >= self.size or new_snake_head[1] >= self.size:
        self.snake.alive = False
      # Check if snake eats itself
      elif new_snake_head == old_snake_tail:
        self.snake.alive = False
      # Check if snake eats the food
      if old_snake_tail == self.food_position:
        # Remove old food
